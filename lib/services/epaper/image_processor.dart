...
class ProcessResultBytes {
  ...
  final Uint8List thumbnailJpg; // JPEG thumbnail of original for album upload
  final Uint8List epdgz;

  const ProcessResultBytes({
    required this.previewPng,
    required this.originalPng,
    required this.thumbnailJpg,
    required this.epdgz,
  });
}
...
ProcessResultBytes _processInIsolate(_IsolateParams p) {
  ...
  // Generate JPEG thumbnail from the original (resized to max 400px)
  final thumbMaxDim = 400;
  final origImg = preview.original;
  ...
  return ProcessResultBytes(
    ...
    thumbnailJpg: Uint8List.fromList(img.encodeJpg(thumb, quality: 85)),
    epdgz: device.epdgz,
  );
}
